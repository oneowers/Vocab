...
async def run_test():
    ...
    try:
        ...
        # -> Attempt to sign in using provided credentials so we can then sign out and verify protected-page access is blocked afterward.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div[2]/form/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('kamolov1575@gmail.com')
        ...
        # -> Attempt to sign in by filling the email and password fields and submitting the login form, then observe the resulting page state.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Submit the login form (click 'Log in' button index 927). After the page changes, locate the app shell and trigger the sign-out control.
        frame = context.pages[-1]
        # Input text
        ...
        assert '/login' in current_url, "The page should have navigated to the login page after signing out from the app shell."
        current_url = await frame.evaluate("() => window.location.href")
        assert '/login' in current_url, "The page should have navigated to the login page when attempting to access a protected route after signing out."
